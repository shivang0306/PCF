...
class CalloutControlComponent extends React.Component<iPropsInput> {
  ...
  _onchange: (currentValue?: string | null) => void;
  _isDisabled: boolean;
  ...
  constructor(props: iPropsInput) {
    ...
    this._isDisabled = props.isDisabled;
    ...
    this._ddlView =
      ...
      this._tmpField.name +
      styles[this._tmpField?.focusControl ?? "focusControl"];
    this._divValidations =
      "divValidations" +
      this._tmpField.name +
      styles[this._tmpField?.focusControl ?? "focusControl"];
    this._linkLookupText =
      "linkLookupText" +
      this._tmpField.name +
      styles[this._tmpField?.focusControl ?? "focusControl"];
    this._divTextbox =
  ...
  LoadInitialData = () => {
    ...
    if (
      ...
      this._entityId != "00000000-0000-0000-0000-000000000000"
    ) {
      this._context.webAPI
        .retrieveRecord(
          ...
          "?$select=_" +
            ...
            "_value&$expand=" +
            this._tmpField.schemaName +
            "($select=" +
            thisRef._tmpField.lookUpCol?.displayField +
            ")"
        )
        .then(
          function success(result) {
            ...
            const tmpLookupTextDisplay =
              result["" + thisRef._tmpField.schemaName + ""] != null
                ? result["" + thisRef._tmpField.schemaName + ""][
                    "" + thisRef._tmpField.lookUpCol?.displayField + ""
                  ]
                : undefined;
            ...
            thisRef.setState({
              ...
              lookupTextDisplay: tmpLookupTextDisplay,
      ...
      const divLookup = document.getElementById(thisRef._divLookupId);
      const divTextbox = document.getElementById(thisRef._divTextbox);
      const divValidations = document.getElementById(thisRef._divValidations);
  ...
  LoadColumns = () => {
    ...
    this._tmpField.lookUpCol?.fieldsToShow?.forEach(
      (tmp: iField, index: number) => {
        const tmpCol = {
          title: tmp.displayText,
          field: tmp.name,
          headerFilter: "input",
          ...(index === 0 && { formatter: this.FirstColumnFormatter })
        };
  ...
  FirstColumnFormatter = (cell: any) => {
    ...
    div.onclick = (e) => {
      ...
      const tmpSelectedItem =
        rowData[thisRef._tmpField.lookUpCol?.primaryFeild ?? ""];
      const tmpSelectedItemDisplay =
        rowData[thisRef._tmpField.lookUpCol?.displayField ?? ""];
      const tmpSelectedItemId =
        rowData[thisRef._tmpField.lookUpCol?.primaryKey ?? ""];

      thisRef.setState({
        lookupText: tmpSelectedItem,
        lookupTextDisplay: tmpSelectedItemDisplay,
      ...
      thisRef._onchange(tmpSelectedItem);
  ...
  LoadDataFromNonFetchXML = () => {
    ...
    tmpSelect = `${tmpSelect + this._tmpField.lookUpCol?.primaryKey},${
      this._tmpField.lookUpCol?.displayField
  ...
  PopulateData = (result: any) => {
    ...
    for (let i = 0; i < result.entities.length; i++) {
      ...
      tmpItem[this._tmpField.lookUpCol?.primaryFeild ?? ""] =
        result.entities[i][this._tmpField.lookUpCol?.primaryFeild ?? ""];
      tmpItem[this._tmpField.lookUpCol?.displayField ?? ""] =
        result.entities[i][this._tmpField.lookUpCol?.displayField ?? ""];
      this._tmpField.lookUpCol?.fieldsToShow?.forEach((tmp: iField) => {
        if (tmp.name) {
  ...
  SetLookupText = () => {
    ...
    if (linkLookupText)
      linkLookupText.innerText = this.state.lookupTextDisplay ?? "";
  };
    ...
    this._context.navigation
      ...
      .then(
        function (success) {
          ...
          const savedRef = success.savedEntityReference;

          let tmpSelect = "?$select=";
          thisRef._tmpField.lookUpCol?.fieldsToShow?.forEach((tmp: iField) => {
            tmpSelect = tmpSelect + tmp.name + ",";
          });
          tmpSelect = `${tmpSelect + thisRef._tmpField.lookUpCol?.primaryKey},${
            thisRef._tmpField.lookUpCol?.displayField
          },${thisRef._tmpField.lookUpCol?.primaryFeild}`;

          thisRef._context.webAPI
            .retrieveRecord(
              (success.savedEntityReference[0] as any).entityType,
            ...
            .then(
              function success(result) {
                const tmpLookupId =
                  result["_" + thisRef._tmpField.name + "_value"];
                const tmpLookupText =
                  result[
                    "_" +
                      thisRef._tmpField.name +
                ...
                const tmpLookupTextDisplay =
                  result["" + thisRef._tmpField.name + ""][
                    "" + thisRef._tmpField.lookUpCol?.displayField + ""
                  ];

                const lookupValue = new Array();
                lookupValue[0] = new Object();
                lookupValue[0].id = tmpLookupId;
                lookupValue[0].name = tmpLookupText;
                lookupValue[0].entityType = (savedRef[0] as any).entityType;

                // @ts-ignore
                const tmpLookupField = Xrm.Page.getAttribute(
                  thisRef._tmpField.name
                );
                tmpLookupField.setValue(lookupValue);

                thisRef._onchange(tmpLookupText);
                                        ...
                                        <td align="left">
                                          {this._tmpField.newRecordText && (
                                            ...
                                            >
                                              <span className="gl_span">
                                                <span className="gl_span_span">
                                                  <span className="glNewFont gl_span_span_icon symbolFont New-symbol"></span>
                                                </span>
                                                <span className="editable_grid_actions_span_span_label alignContentCenter" >
                                                  {this._tmpField
                                                    .newRecordText ?? "New"}
                                                </span>
                                              </span>
