...
export const IncidentStatus: React.FunctionComponent<IncidentStatusProps> = (
    props: IncidentStatusProps
) => {
    ...
    React.useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const data = await fetchDataFromAPI(props.itemId);
                setData(data);
                setLoader(() => {
                    return false;
                });
    ...
    async function fetchDataFromAPI(id: string | undefined): Promise<htmlComponentProp[]> {
        const data: htmlComponentProp[] = [];
        const workorderincidents = await props.context.webAPI.retrieveMultipleRecords(
        ...
        for (let i = 0; i < workorderincidents.entities.length; i++) {

            const obj: htmlComponentProp = {
    ...
    return (
        <>
            {
                loader ?
                    <ThreeDot color="#115ea3" size="small" text="" textColor="#115ea3" /> :
                    <div
                        ref={scrollRef}
                        style={{
                            display: "flex",
